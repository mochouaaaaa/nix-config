...
function print-unicode-ranges() {
    ...
    for ((j = 0; j < len; j += 2)); do
        ...
        while IFS='' read -r -t 1 combinedRange; do
            # Process each combined range here
            echo "$combinedRange"
        done < <(seq "$startDecimal" "$endDecimal")

        # for combinedRange in $(seq "$startDecimal" "$endDecimal"); do
        #     # Process each combined range here
        #     echo "Combined Range: $combinedRange"
        # done
