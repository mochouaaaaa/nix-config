...
in
{
  config = lib.mkIf (cfg.enable && cfg.settings.templates.btop) {
    ...
    programs = {
      btop = {
        settings = {
          color_theme = lib.mkForce "matugen";
        };
      };
    };
    xdg.configFile."btop/themes".enable = false;

    programs.noctalia-shell.user-templates = {
      templates = {
        btop = {
          inputPath = "${config.xdg.configHome}/noctalia/colorschemes/btop.theme";
    ...
    xdg.configFile = {
      "noctalia/colorschemes/btop.theme".text = ''
